...
%Constraint
LB     = [1 1 1];          %batas bawah RLDC RLUS RLUC
UB     = [128 128 128];         %batas atas 


% UB = [0.9 0.02224 7];% Upper Bounds diganti sesuai dengan constraint fungsi objektif
% LB = [0.6 0.0127 5];% Lower Bounds diganti sesuai dengan constraint fungsi objektif
%%
CostFunction=@(x) (model1(x));        % Cost Function %Model
...
% If you would like to use Constriction Coefficients for KWA,
...
title('Grafik Nilai Minimum KWA Jarak 10Km','color','b')
xlabel('Jumlah Iterasi');
ylabel('Nilai Fungsi Obyektif (ms)');
grid on;